...
# Run plink2 commands directly on command line to conduct PCA for autosomes and chrY
...
YEigenPercent <- round((head(YEigenValues, n = 22L) / (sum(head(YEigenValues, n = 22L)))*100), 2)
...
AutoPlot = ggplot(data = AutoEigenVectors) +
  ...
  labs(title = "Autosomes", x = paste0("PC1 (",AutoEigenPercent[3,1]," %)"),
       y = paste0("PC2 (",AutoEigenPercent[4,1]," %)"),
